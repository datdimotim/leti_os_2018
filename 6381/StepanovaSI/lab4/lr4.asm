...
ROUT_CALC:	
	...
	mov AX,SEG COUNTER
	mov DS,AX
	mov SI,offset COUNTER ;для изменения строки
	...
	mov [SI-1],BH ;возвращаем
	;(0*00) 
	mov CH,[SI-2] ;получаем цифру
	inc CH ;увеличиваем её на 1
	mov [SI-2],CH ;возвращаем
	cmp CH,3Ah ;если не больше 9 
	jne END_CALC ;заканчиваем
	mov CH,30h ;обнуляем
	mov [SI-2],CH ;возвращаем
	;(*000) 
	mov DH,[SI-3] ;получаем цифру
	inc DH ;увеличиваем её на 1
	mov [SI-3],DH ;возвращаем
	cmp DH,3Ah ;если не больше 9 
	jne END_CALC ;заканчиваем
	mov DH,30h ;обнуляем
	mov [SI-3],DH ;возвращаем	
ROUT_REC1:
...
ROUT_REC:
	...
	mov AL,1Ch ;AL = номер вектора прерывания
...
UNLOAD: ;если /un
	pop AX
	pop ES
	mov byte ptr ES:[BX+SI+10],1 ;DELETION = 1
	mov dx,offset UNLOADED ;вывод сообщения
	call PRINT
	ret
CHECK_INT ENDP
